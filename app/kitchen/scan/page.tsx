        ...
        <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {distributors.map(d => (
                <button key={d.id} onClick={() => setSelectedId(d.id)} style={{
                  ...
                  border: selectedId === d.id ? '2px solid var(--fmb-accent)' : '2px solid #1e293b',
                  background: selectedId === d.id ? '#1e293b' : '#0f172a',
                  color: selectedId === d.id ? theme.accent : '#94a3b8',
                  ...
                  <div style={{
                    ...
                    background: selectedId === d.id ? 'var(--fmb-accent)20' : '#1e293b',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 18, color: selectedId === d.id ? theme.accent : '#475569',
                  }}>
                    {selectedId === d.id ? '✓' : '○'}
                  </div>
                  {d.full_name}
          ...
          {!autoDemo && distributors.length > 1 && (
            <div style={{ width: '100%', maxWidth: 340, marginBottom: 24 }}>
              <button onClick={() => { setAutoDemo(true); setAutoDemoIdx(0); }} style={{
                ...
                background: '#1e293b', color: theme.accent, fontSize: 14, fontWeight: 700,
          ...
          {autoDemo && (
            <div style={{ width: '100%', maxWidth: 340, marginBottom: 24, textAlign: 'center' }}>
              <div style={{ color: theme.accent, fontWeight: 700, fontSize: 15, marginBottom: 6 }}>
          ...
          <div style={{ position: 'relative' }}>
            {(tapping || autoDemo) && (
              <>
                <div style={{ position: 'absolute', inset: -20, borderRadius: '50%', border: '2px solid var(--fmb-accent)', opacity: 0, animation: 'ripple 1s ease-out forwards' }} />
                <div style={{ position: 'absolute', inset: -40, borderRadius: '50%', border: '2px solid var(--fmb-accent)', opacity: 0, animation: 'ripple 1s ease-out 0.2s forwards' }} />
              </>
            )}
            <button
              ...
              style={{
                ...
                background: tapping ? 'radial-gradient(circle, var(--fmb-accent), #f97316)' :
                  result === 'success' ? 'radial-gradient(circle, #22c55e, #16a34a)' :
                  result === 'error'   ? 'radial-gradient(circle, #ef4444, #dc2626)' :
                  'radial-gradient(circle, var(--fmb-primary), #1e3a5f)',
                boxShadow: tapping ? '0 0 60px var(--fmb-accent)80' : result === 'success' ? '0 0 60px #22c55e80' : '0 0 40px var(--fmb-primary)40',
