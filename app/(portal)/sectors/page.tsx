...
export default function SectorsPage() {
  ...
  const toggleStatus = async (sector: Sector) => {
    ...
    await supabase.from('house_sectors').update({ status: newStatus }).eq('id', sector.id)
    await fetchSectors()
  }
    ...
    <div>
      ...
      <div className="card" style={{ border: 'none', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', borderRadius: '10px' }}>
        <div className="card-body">
          ...
          {loading ? (
            ...
            <table className="table table-hover mb-0">
              <thead style={{ background: '#f8f9fa' }}>
                <tr>
                  <th style={{ fontSize: '13px', color: '#6c757d', fontWeight: 600 }}>#</th>
                  <th style={{ fontSize: '13px', color: '#6c757d', fontWeight: 600 }}>Sector Name</th>
                  <th style={{ fontSize: '13px', color: '#6c757d', fontWeight: 600 }}>Status</th>
                  <th style={{ fontSize: '13px', color: '#6c757d', fontWeight: 600 }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={sector.id}>
                    ...
                    <td>
                      <button className="btn btn-sm btn-outline-primary me-2"
                        style={{ fontSize: '12px' }} onClick={() => openEdit(sector)}>
                        Edit
                      </button>
          ...
          <div className="mt-2">
            <small className="text-muted">{filtered.length} sectors</small>
          </div>
        </div>
      </div>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                ...
                <input
                  ...
                  placeholder="e.g. AHMED MARKET SF-12"
                  autoFocus
                />
              </div>
