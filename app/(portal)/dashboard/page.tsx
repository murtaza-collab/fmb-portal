...
interface TodaySchedule {
  ...
  total_thaalis: number   // computed from approved registrations + extra
}
...
export default function DashboardPage() {
  ...
  const fetchStats = async () => {
    ...
    if (sched) {
      // Count approved thaalis for today
  ...
  const kitchenFlow = [
    ...
    { label:'Filling',       value:kitchenStats.inProgress + kitchenStats.counterBDone + kitchenStats.counterCDone,
                                                              color:'#ffbf69', bg:'#ffbf6918', icon:'bi-cup-hot', sub:`${kitchenStats.totalThaalisInProcess} thaalis` },
    { label:'Ready',         value:kitchenStats.counterBDone + kitchenStats.counterCDone,
                                                              color:'#0ab39c', bg:'#0ab39c18', icon:'bi-check2-circle', sub:`${kitchenStats.totalThaalisReady} thaalis` },
  ...
  return (
    <>
        ...
        <div className="col-xl-3 col-md-6">
          <div className="card h-100" style={{ borderRadius:12, border:'none', boxShadow:'0 1px 6px rgba(0,0,0,0.07)' }}>
            <div className="card-body p-3">
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  <p style={{ fontSize:11, textTransform:'uppercase', letterSpacing:'0.6px', fontWeight:600, color:'var(--bs-secondary-color)', marginBottom:4 }}>Active Thaalis</p>
                  <h3 className="mb-0 fw-bold" style={{ color:'#0ab39c', fontSize:28 }}>{stats.activeThaalis}</h3>
                  <p style={{ fontSize:12, color:'var(--bs-secondary-color)', marginTop:4, marginBottom:0 }}>
                    {stats.stoppedThaalis > 0
                      ? <span style={{ color:'#f06548' }}>{stats.stoppedThaalis} stopped</span>
                      : '0 stopped'}
                  </p>
                </div>
      ...
      {/* ── Small info strip: Sectors + Distributors ── */}
            ...
            <div className="card-body p-3">
              ...
              {todaySchedule ? (
                ...
                <div className="mb-2 px-2 py-2 rounded d-flex align-items-center gap-2"
                  ...
                  <div style={{ fontSize:12, color:'var(--bs-secondary-color)' }}>Not scheduled — <button onClick={() => router.push('/calendar')} className="btn btn-link p-0" style={{ fontSize:12 }}>Add</button></div>
                </div>
              )}
