...
export default function DistributorsPage() {
  ...
  const fetchDistributors = async () => {
    setLoading(true)
    const { data } = await supabase.from('distributors').select('*').order('full_name')
    setDistributors(data || [])
    setLoading(false)
  }

  const fetchSectors = async () => {
    const { data } = await supabase.from('house_sectors').select('*').eq('status', 'active').order('name')
    setSectors(data || [])
  }
  ...
  return (
    <div>
                ...
                <tbody>
                  {filtered.map((d, i) => (
                    <tr key={d.id}>
                      ...
                      <td style={{ fontSize: '14px', whiteSpace: 'nowrap' }}>{d.full_name}</td>
                      <td style={{ fontSize: '13px' }}>{d.username}</td>
                      <td style={{ fontSize: '13px' }}>{d.sabeel_no || '—'}</td>
                      <td style={{ fontSize: '13px', whiteSpace: 'nowrap' }}>{d.phone_no || '—'}</td>
                      <td>
              ...
              <div className="modal-body" style={{ maxHeight: '65vh', overflowY: 'auto' }}>
                <div className="row g-3">
                  {[
                    ...
                    <div key={field.key} className="col-12 col-sm-6">
      ...
      {showSectorModal && selectedDistributor && (
        <div className="modal show d-block" style={{ background: 'rgba(0,0,0,0.5)' }} onClick={() => setShowSectorModal(false)}>
          <div className="modal-dialog modal-lg" onClick={e => e.stopPropagation()}>
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title" style={{ fontSize: '15px' }}>Assign Sectors — {selectedDistributor.full_name}</h5>
              ...
              <div className="modal-body" style={{ maxHeight: '65vh', overflowY: 'auto' }}>
                ...
                <div className="row g-2">
                  {sectors.map(sector => (
                    <div key={sector.id} className="col-12 col-sm-6">
                      <div
