...
interface KitchenSettings {
  customization_cutoff_hours: number
  stop_thaali_cutoff_hours: number
}
...
const LOOKUP_TABLE: Partial<Record<TabKey, string>> = {
  blocks: 'house_blocks', types: 'house_types', niyyat: 'niyyat_statuses',
  mumin_categories: 'mumin_categories', thaali_types: 'thaali_types', thaali_categories: 'thaali_categories',
}
...
export default function SettingsPage() {
  ...
  const [thaaliFilter, setThaaliFilter]   = useState<'all' | 'unassigned' | 'assigned'>('all')
  const [kitchenSettings, setKitchenSettings] = useState<KitchenSettings>({
    customization_cutoff_hours: 36,
    stop_thaali_cutoff_hours: 48,
  })
  ...
  const loadTab = async (tab: TabKey) => {
    ...
    if (tab === 'fiscal') {
      ...
      const { data } = await supabase.from('kitchen_settings')
        .select('setting_key, setting_value')
        .in('setting_key', ['customization_cutoff_hours', 'stop_thaali_cutoff_hours'])
      const customizationRow = (data || []).find((d: any) => d.setting_key === 'customization_cutoff_hours')
      const stopThaaliRow    = (data || []).find((d: any) => d.setting_key === 'stop_thaali_cutoff_hours')
      setKitchenSettings({
        customization_cutoff_hours: customizationRow ? parseInt(customizationRow.setting_value) || 36 : 36,
        stop_thaali_cutoff_hours:   stopThaaliRow    ? parseInt(stopThaaliRow.setting_value)    || 48 : 48,
      })
    } else if (tab === 'sectors') {
      ...
      const { data: dsData } = await supabase.from('distributor_sectors')
        .select('sector_id, distributor_id, distributors(id, full_name)').in('sector_id', secIds)
      ...
      // ── FIXED: limit 6000 to load all thaali numbers ──
      const { data: thaalis } = await supabase.from('thaalis')
        .select('id, thaali_number').order('thaali_number').limit(6000)
      if (!thaalis?.length) { setThaaliNumbers([]); setLoading(false); return }
      const { data: regs } = await supabase.from('thaali_registrations')
        .select('thaali_id').not('thaali_id', 'is', null)
  ...
  const openAdd = () => {
    setEditRow(null); setFormName(''); setFormColour('#198754'); setFormDescription(''); setShowModal(true)
  }
  const openEdit = (row: LookupRow) => {
    setEditRow(row); setFormName(row.name); setFormColour(row.colour || '#198754'); setFormDescription(row.description || ''); setShowModal(true)
  }
  const saveRow = async () => {
  ...
  const saveThaaliNumber = async () => {
    ...
    setSaving(false)
    if (error) return showMsg(error.message, true)
    setShowThaaliModal(false); setThaaliInput(''); showMsg(`Added ${nums.length} thaali number(s)`); loadTab('thaali_numbers')
  }
  ...
  const saveKitchenSettings = async () => {
    ...
    try {
      await supabase.from('kitchen_settings').upsert(
        { setting_key: 'customization_cutoff_hours', setting_value: kitchenSettings.customization_cutoff_hours.toString(), updated_at: new Date().toISOString() },
        { onConflict: 'setting_key' }
      )
      await supabase.from('kitchen_settings').upsert(
        { setting_key: 'stop_thaali_cutoff_hours', setting_value: kitchenSettings.stop_thaali_cutoff_hours.toString(), updated_at: new Date().toISOString() },
        { onConflict: 'setting_key' }
      )
      showMsg('✅ All cutoff settings saved successfully!')
    } catch (err: any) {
      showMsg('Error saving: ' + err.message, true)
    } finally { setSaving(false) }
  }
  const formatHoursDisplay = (hours: number) => {
    const days = Math.floor(hours / 24); const remainder = hours % 24
    if (days > 0 && remainder === 0) return `${days} day${days > 1 ? 's' : ''} before`
    else if (days > 0) return `${days} day${days > 1 ? 's' : ''} and ${remainder}h before`
    else return `${remainder} hours before`
  }
                ...
                <div key={group} className="mb-2">
                  ...
                  {TABS.filter((t) => t.group === group).map((tab) => (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)} className="w-100 text-start btn btn-sm"
                      style={{ borderRadius: '8px', marginBottom: '2px', background: activeTab === tab.key ? 'var(--fmb-primary)' : 'transparent',
            ...
            <div className="card-body p-3">
              ...
              <div className="d-flex align-items-center justify-content-between mb-3">
                <div className="d-flex align-items-center gap-2">
                  <i className={`bi ${currentTab?.icon}`} style={{ fontSize: '18px', color: 'var(--fmb-primary)' }} />
                ...
                {showAddButton && (
                  <div className="d-flex gap-2">
                    ...
                    <button onClick={() => (activeTab === 'thaali_numbers' ? setShowThaaliModal(true) : openAdd())}
                      className="btn btn-sm" style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}>
              ...
              {loading ? (
                <div className="text-center py-5"><div className="spinner-border spinner-border-sm" style={{ color: 'var(--fmb-primary)' }} /></div>
              ) : activeTab === 'fiscal' ? (
                <div>
                  <p style={{ fontSize: '12px', color: 'var(--bs-secondary-color)' }} className="mb-3">
                    <i className="bi bi-info-circle me-1" />Auto-generated from Misri Hijri calendar. FMB fiscal year: 1 Ramadan → 29 Shaban.
                  </p>
                  <div className="table-responsive">
                    <table className="table table-hover" style={{ fontSize: '13px' }}>
                      <thead style={{ background: 'var(--bs-tertiary-bg)', borderBottom: '2px solid var(--bs-border-color)' }}>
                        <tr>
                          {['Hijri Year','Ramadan Start','Year End','Gregorian Span','Status'].map(h => (
                            <th key={h} style={{ color: 'var(--bs-secondary-color)', fontWeight: 700 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {computedFYs.map((fy) => (
                          <tr key={fy.hijriYear} style={{ background: fy.isCurrentFY ? 'var(--fmb-primary)08' : undefined }}>
                            <td>
                              <span style={{ fontWeight: 700, color: fy.isCurrentFY ? 'var(--fmb-primary)' : 'var(--bs-body-color)' }}>{fy.hijriYear}H</span>
                              {fy.isCurrentFY && <span className="badge ms-2" style={{ background: 'var(--fmb-primary)20', color: 'var(--fmb-primary)', fontSize: '10px' }}>Current</span>}
                            </td>
                            ...
                            <td>
                              {fy.isCurrentFY
                                ...
                                : fy.startGregorian > new Date()
                                  ? <span className="badge" style={{ background: 'var(--fmb-accent)20', color: '#856404', fontWeight: 600, fontSize: '11px' }}>Upcoming</span>
                ...
                <div style={{ maxWidth: 800 }}>
                  ...
                  <div className="card mb-4 border-0 shadow-sm">
                    <div className="card-header" style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: '8px 8px 0 0' }}>
                    ...
                    <div className="card-body p-4">
                      ...
                      <div className="mb-3">
                        <label style={{ fontSize: '12px', fontWeight: 600, color: 'var(--bs-secondary-color)', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 8 }}>Quick Select</label>
                          ...
                          {CUTOFF_PRESETS.map((h) => (
                            <button key={h} onClick={() => setKitchenSettings({ ...kitchenSettings, customization_cutoff_hours: h })} className="btn btn-sm"
                              style={{ borderRadius: 20, fontSize: 13, fontWeight: 600, padding: '5px 16px',
                                background: kitchenSettings.customization_cutoff_hours === h ? 'var(--fmb-primary)' : 'var(--bs-tertiary-bg)',
                      ...
                      <div className="mb-3">
                        <label style={{ fontSize: '12px', fontWeight: 600, color: 'var(--bs-secondary-color)', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 8 }}>Or enter custom hours</label>
                        <div className="input-group" style={{ maxWidth: 200 }}>
                          <input type="number" min={1} max={168} className="form-control"
                            value={kitchenSettings.customization_cutoff_hours}
                            onChange={(e) => setKitchenSettings({ ...kitchenSettings, customization_cutoff_hours: parseInt(e.target.value) || 36 })}
                      ...
                      <div className="p-3" style={{ background: 'var(--bs-info-bg-subtle)', borderRadius: 10, border: '1px solid var(--bs-info-border-subtle)' }}>
                        <div style={{ fontSize: 13, color: 'var(--bs-info-text-emphasis)' }}>
                          <i className="bi bi-info-circle me-2" /><strong>Current: {kitchenSettings.customization_cutoff_hours} hours</strong> before delivery
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--bs-info-text-emphasis)', marginTop: 6, paddingLeft: 22 }}>
                          Example: for a <strong>Wednesday</strong> delivery, cutoff is <strong>{formatHoursDisplay(kitchenSettings.customization_cutoff_hours)}</strong>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Stop Thaali Cutoff */}
                  <div className="card mb-4 border-0 shadow-sm">
                    <div className="card-header" style={{ background: '#198754', color: '#fff', borderRadius: '8px 8px 0 0' }}>
                      <h6 className="mb-0 fw-bold"><i className="bi bi-pause-circle me-2" />Stop Thaali Cutoff</h6>
                    </div>
                    <div className="card-body p-4">
                      <p style={{ fontSize: '13px', color: 'var(--bs-secondary-color)' }} className="mb-3">
                        How many hours before delivery can mumineen request to temporarily stop their thaali.
                      </p>
                      <div className="mb-3">
                        <label style={{ fontSize: '12px', fontWeight: 600, color: 'var(--bs-secondary-color)', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 8 }}>Quick Select</label>
                        <div className="d-flex flex-wrap gap-2">
                          {CUTOFF_PRESETS.map((h) => (
                            <button key={h} onClick={() => setKitchenSettings({ ...kitchenSettings, stop_thaali_cutoff_hours: h })} className="btn btn-sm"
                              style={{ borderRadius: 20, fontSize: 13, fontWeight: 600, padding: '5px 16px',
                                background: kitchenSettings.stop_thaali_cutoff_hours === h ? '#198754' : 'var(--bs-tertiary-bg)',
                                color: kitchenSettings.stop_thaali_cutoff_hours === h ? '#fff' : 'var(--bs-body-color)',
                                border: kitchenSettings.stop_thaali_cutoff_hours === h ? 'none' : '1px solid var(--bs-border-color)' }}>
                              {h}h
                            </button>
                          ))}
                        </div>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="input-group" style={{ maxWidth: 200 }}>
                          <input type="number" min={1} max={168} className="form-control"
                            value={kitchenSettings.stop_thaali_cutoff_hours}
                            onChange={(e) => setKitchenSettings({ ...kitchenSettings, stop_thaali_cutoff_hours: parseInt(e.target.value) || 48 })}
                            style={{ borderRadius: '8px 0 0 8px', fontSize: 14, fontWeight: 600 }} />
                          <span className="input-group-text" style={{ borderRadius: '0 8px 8px 0', fontSize: 13, color: 'var(--bs-secondary-color)' }}>hours</span>
                        </div>
                      </div>
                      <div className="p-3" style={{ background: 'var(--bs-info-bg-subtle)', borderRadius: 10, border: '1px solid var(--bs-info-border-subtle)' }}>
                        <div style={{ fontSize: 13, color: 'var(--bs-info-text-emphasis)' }}>
                          <i className="bi bi-info-circle me-2" /><strong>Current: {kitchenSettings.stop_thaali_cutoff_hours} hours</strong> before delivery
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--bs-info-text-emphasis)', marginTop: 6, paddingLeft: 22 }}>
                          Example: for a <strong>Wednesday</strong> delivery, stop request must be submitted <strong>{formatHoursDisplay(kitchenSettings.stop_thaali_cutoff_hours)}</strong>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <button onClick={saveKitchenSettings} disabled={saving} className="btn btn-lg w-100"
                    style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: '8px', fontSize: '14px', fontWeight: 600 }}>
                  ...
                  <div className="alert alert-warning mt-3 mb-0" style={{ fontSize: '12px' }}>
                    <i className="bi bi-exclamation-triangle me-2" /><strong>Note:</strong> Legacy time-based settings are deprecated.
                  </div>
                </div>
                ...
                <>
                  ...
                  <div className="d-flex align-items-center gap-2 mb-3 flex-wrap">
                    {[
                      { key: 'all',        label: `All (${thaaliNumbers.length})`,                                  color: 'var(--fmb-primary)' },
                      { key: 'unassigned', label: `Unassigned (${thaaliNumbers.filter(t => !t.assigned).length})`, color: '#0ab39c' },
                      { key: 'assigned',   label: `Assigned (${thaaliNumbers.filter(t => t.assigned).length})`,    color: 'var(--fmb-primary)' },
                    ].map(tab => (
                      <button key={tab.key} onClick={() => { setThaaliFilter(tab.key as any); setThaaliPage(1) }} className="btn btn-sm"
                        style={{ borderRadius: 20, fontSize: 12, fontWeight: 600, padding: '4px 14px',
                          ...
                          border: thaaliFilter === tab.key ? 'none' : '1px solid var(--bs-border-color)' }}>
                        {tab.label}
                      </button>
                    ))}
                      ...
                      <tbody>
                        {pagedThaaliNumbers.length === 0 ? (
                          ...
                          <tr key={t.id}>
                            ...
                            <td>
                              {t.assigned
                                ? <span className="badge" style={{ background: 'var(--fmb-primary)20', color: 'var(--fmb-primary)', fontWeight: 600, fontSize: '11px' }}>Assigned</span>
                                : <span className="badge" style={{ background: '#0ab39c20', color: '#0ab39c', fontWeight: 600, fontSize: '11px' }}>Unassigned</span>}
                            </td>
                            <td>
                              <button onClick={() => deleteThaali(t.id)} disabled={t.assigned}
                                title={t.assigned ? 'Cannot delete — assigned to a mumin' : 'Delete'} className="btn btn-sm"
                                style={{ padding: '3px 8px', background: t.assigned ? 'transparent' : '#e6394615',
                                  color: t.assigned ? 'var(--bs-secondary-color)' : '#e63946', border: 'none', borderRadius: '6px' }}>
                        ...
                        {(() => {
                          ...
                          return pages.map((p, i) => p < 0
                            ...
                            : <button key={p} onClick={() => setThaaliPage(p)} className="btn btn-sm" style={{ borderRadius: 6, fontSize: 12, minWidth: 32, padding: '3px 8px',
                                background: thaaliPage === p ? 'var(--fmb-primary)' : 'var(--bs-tertiary-bg)',
                    ...
                    <div key={sec.id} className="mb-2" style={{ border: '1px solid var(--bs-border-color)', borderRadius: 10, overflow: 'hidden' }}>
                      <div className="d-flex align-items-center px-3 py-2" style={{ background: 'var(--bs-body-bg)' }}>
                        ...
                        <button className="btn btn-sm me-2"
                          style={{ background: sec.distributors.length > 0 ? 'var(--fmb-primary)15' : 'var(--bs-tertiary-bg)',
                            color: sec.distributors.length > 0 ? 'var(--fmb-primary)' : 'var(--bs-secondary-color)',
                          ...
                          <i className={`bi ms-1 ${expandedSector === sec.id ? 'bi-chevron-up' : 'bi-chevron-down'}`} style={{ fontSize: 10 }} />
                        </button>
                        <div className="d-flex gap-1">
                          <button onClick={() => openEdit(sec)} className="btn btn-sm"
                            style={{ padding: '3px 8px', background: 'var(--fmb-primary)15', color: 'var(--fmb-primary)', border: 'none', borderRadius: '6px' }}>
                            <i className="bi bi-pencil" style={{ fontSize: '11px' }} />
                          </button>
                          <button onClick={() => deleteRow(sec.id)} className="btn btn-sm"
                            style={{ padding: '3px 8px', background: '#e6394615', color: '#e63946', border: 'none', borderRadius: '6px' }}>
                            <i className="bi bi-trash" style={{ fontSize: '11px' }} />
                          </button>
                        </div>
                      </div>
                        ...
                        <div style={{ background: 'var(--bs-tertiary-bg)', borderTop: '1px solid var(--bs-border-color)', padding: '8px 12px' }}>
                          {sec.distributors.length === 0 ? (
                            <p className="mb-0" style={{ fontSize: 12, color: 'var(--bs-secondary-color)', fontStyle: 'italic' }}>
                              No distributors assigned. Assign from the Distributors page.
                            </p>
                          ) : sec.distributors.map((d) => (
                            <div key={d.id} className="d-flex align-items-center py-1">
                              <i className="bi bi-person-fill me-2" style={{ fontSize: 12, color: 'var(--bs-secondary-color)' }} />
                              <span style={{ fontSize: 13, color: 'var(--bs-body-color)', flex: 1 }}>{d.full_name}</span>
                              <button onClick={() => removeDistributorFromSector(sec.id, d.id)} className="btn btn-sm"
                        ...
                        {(() => {
                          ...
                          return pages.map((p, i) => p < 0
                            ...
                            : <button key={p} onClick={() => setSectorPage(p)} className="btn btn-sm" style={{ borderRadius: 6, fontSize: 12, minWidth: 32, padding: '3px 8px',
                                background: sectorPage === p ? 'var(--fmb-primary)' : 'var(--bs-tertiary-bg)',
                ...
                <>
                  <div className="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                    <input type="text" placeholder="Search name, SF#, thaali#..." value={stickerSearch}
                      onChange={(e) => setStickerSearch(e.target.value)} className="form-control form-control-sm"
                    ...
                    <div className="d-flex gap-2 align-items-center">
                      ...
                      <button onClick={generateStickers} disabled={generatingPDF || !selectedStickers.length} className="btn btn-sm"
                        style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}>
                    ...
                    <table className="table table-hover" style={{ fontSize: '13px' }}>
                      <thead style={{ background: 'var(--bs-tertiary-bg)', position: 'sticky', top: 0, borderBottom: '2px solid var(--bs-border-color)' }}>
                        <tr>
                          <th style={{ width: '40px' }}>
                            <input type="checkbox"
                              checked={selectedStickers.length === filteredStickers.length && filteredStickers.length > 0}
                              onChange={toggleAll} />
                          </th>
                          {['Thaali #','Name','SF#'].map(h => <th key={h} style={{ color: 'var(--bs-secondary-color)', fontWeight: 700 }}>{h}</th>)}
                        </tr>
                      </thead>
                      <tbody>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '60px 16px' }}
          ...
          <div style={{ background: 'var(--bs-body-bg)', borderRadius: '12px', width: '100%', maxWidth: '440px', boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--bs-border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h6 className="mb-0 fw-bold" style={{ color: 'var(--bs-body-color)' }}>{editRow ? 'Edit' : 'Add'} {currentTab?.label}</h6>
            ...
            <div style={{ padding: '20px' }}>
              <div className="mb-3">
                ...
                <input type="text" className="form-control" value={formName} onChange={(e) => setFormName(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && saveRow()} placeholder="Enter name..."
              ...
              {hasColour && (
                <div className="mb-3">
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <input type="color" className="form-control form-control-color" value={formColour} onChange={(e) => setFormColour(e.target.value)}
                      style={{ width: 48, height: 38, borderRadius: 8, padding: 2 }} />
              ...
              {activeTab === 'mumin_categories' && (
                <div className="mb-3">
                  ...
                  <input type="text" className="form-control" value={formDescription} onChange={(e) => setFormDescription(e.target.value)}
                    placeholder="Optional description..." style={{ borderRadius: 8, fontSize: 13 }} />
                </div>
              )}
            </div>
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--bs-border-color)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={saveRow} disabled={saving} className="btn btn-sm" style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '60px 16px' }}
          ...
          <div style={{ background: 'var(--bs-body-bg)', borderRadius: 12, width: '100%', maxWidth: 440, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}
            ...
            <div style={{ padding: 20 }}>
              ...
              <input type="text" className="form-control" value={thaaliInput} onChange={(e) => setThaaliInput(e.target.value)}
                placeholder="e.g. 1001, 1002, 1003" style={{ borderRadius: 8, fontSize: 13 }} autoFocus />
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--bs-border-color)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={saveThaaliNumber} disabled={saving} className="btn btn-sm" style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600 }}>
          ...
          <div style={{ padding: '10px 16px', background: '#2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <a href={previewUrl} download={`thaali-stickers-${new Date().toISOString().slice(0,10)}.pdf`}
                className="btn btn-sm" style={{ background: 'var(--fmb-primary)', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
                Download PDF
              </a>
