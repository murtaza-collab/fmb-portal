...
const LOOKUP_TABLE: Partial<Record<TabKey, string>> = {
  blocks: 'house_blocks',
  types: 'house_types',
  niyyat: 'niyyat_statuses',
  mumin_categories: 'mumin_categories',
  thaali_types: 'thaali_types',
  thaali_categories: 'thaali_categories',
}
...
export default function SettingsPage() {
  ...
  const loadTab = async (tab: TabKey) => {
    ...
    if (tab === 'fiscal') {
      ...
      // Load BOTH customization and stop thaali cutoff hours
  ...
  const openEdit = (row: LookupRow) => {
    ...
    setFormColour(row.colour || '#198754')
  ...
  const saveRow = async () => {
    ...
    if (activeTab === 'mumin_categories') {
      payload.colour = formColour
  ...
  const saveThaaliNumber = async () => {
    ...
    setSaving(false)
    if (error) return showMsg(error.message, true)
    setShowThaaliModal(false)
    setThaaliInput('')
    showMsg(`Added ${nums.length} thaali number(s)`)
    loadTab('thaali_numbers')
  }
      ...
      await supabase.from('kitchen_settings').upsert(
        {
          ...
          setting_value: kitchenSettings.customization_cutoff_hours.toString(),
      ...
      await supabase.from('kitchen_settings').upsert(
        {
          ...
          setting_value: kitchenSettings.stop_thaali_cutoff_hours.toString(),
          updated_at: new Date().toISOString(),
        },
        { onConflict: 'setting_key' }
      )
  ...
  const formatHoursDisplay = (hours: number) => {
    ...
    if (days > 0 && remainder === 0) {
      ...
      return `${days} day${days > 1 ? 's' : ''} and ${remainder}h before`
    } else {
  ...
  // ── Stickers ──────────────────────────────────────────────────────────────
  const filteredStickers = stickerRegistrations.filter(
    (r: any) =>
      !stickerSearch ||
  ...
  const generateStickers = async () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      doc.rect(x, y, stickerW - 0.1, upperH)
      ...
      doc.setDrawColor(200)
      doc.rect(x, ly, stickerW - 0.1, lowerH)
  ...
  return (
    <>
      ...
      <div className="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div>
          <h4 className="mb-0 fw-bold" style={{ color: 'var(--bs-body-color)' }}>
            Settings
          </h4>
            ...
            <div className="card-body p-3">
              ...
              {loading ? (
                      ...
                      <tbody>
                        {computedFYs.map((fy) => (
                          <tr key={fy.hijriYear} style={{ background: fy.isCurrentFY ? '#36457408' : undefined }}>
                            <td>
                              <span style={{ fontWeight: 700, color: fy.isCurrentFY ? '#364574' : 'var(--bs-body-color)' }}>
                                {fy.hijriYear}H
                              </span>
                              {fy.isCurrentFY && (
                                <span className="badge ms-2" style={{ background: '#36457420', color: '#364574', fontSize: '10px' }}>
                                  Current
                                </span>
                              )}
                            </td>
                            ...
                            <td>
                              {fy.isCurrentFY ? (
                                <span className="badge" style={{ background: '#0ab39c20', color: '#0ab39c', fontWeight: 600, fontSize: '11px' }}>
                                  Active
                                </span>
                              ) : fy.startGregorian > new Date() ? (
                                <span className="badge" style={{ background: '#ffbf6920', color: '#856404', fontWeight: 600, fontSize: '11px' }}>
                                  Upcoming
                                </span>
                              ) : (
                                <span
                                  className="badge"
                                  style={{ background: 'var(--bs-secondary-bg)', color: 'var(--bs-secondary-color)', fontWeight: 600, fontSize: '11px' }}
                                >
                                  Past
                                </span>
                              )}
                ...
                <div style={{ maxWidth: 800 }}>
                  {/* Customization Cutoff Section */}
                  <div className="card mb-4 border-0 shadow-sm">
                    <div
                      className="card-header"
                      style={{ background: '#364574', color: '#fff', borderRadius: '8px 8px 0 0 !important' }}
                    >
                    ...
                    <div className="card-body p-4">
                      ...
                      <div className="mb-3">
                        ...
                        <div className="d-flex flex-wrap gap-2">
                          {CUTOFF_PRESETS.map((h) => (
                            <button
                              key={h}
                              onClick={() => setKitchenSettings({ ...kitchenSettings, customization_cutoff_hours: h })}
                              className="btn btn-sm"
                              style={{
                                borderRadius: 20,
                                fontSize: 13,
                      ...
                      <div className="mb-3">
                        ...
                        <div className="input-group" style={{ maxWidth: 200 }}>
                          <input
                            ...
                            onChange={(e) =>
                              setKitchenSettings({ ...kitchenSettings, customization_cutoff_hours: parseInt(e.target.value) || 36 })
                            }
                            style={{ borderRadius: '8px 0 0 8px', fontSize: 14, fontWeight: 600 }}
                          />
                          <span
                            className="input-group-text"
                            style={{ borderRadius: '0 8px 8px 0', fontSize: 13, color: 'var(--bs-secondary-color)' }}
                          >
                            hours
                          </span>
                        </div>
                      ...
                      <div
                        className="p-3 mb-3"
                        style={{ background: 'var(--bs-info-bg-subtle)', borderRadius: 10, border: '1px solid var(--bs-info-border-subtle)' }}
                      >
                        <div style={{ fontSize: 13, color: 'var(--bs-info-text-emphasis)' }}>
                          <i className="bi bi-info-circle me-2"></i>
                          <strong>Current: {kitchenSettings.customization_cutoff_hours} hours</strong> before delivery
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--bs-info-text-emphasis)', marginTop: 6, paddingLeft: 22 }}>
                          Example: for a <strong>Wednesday</strong> delivery, cutoff is{' '}
                          <strong>{formatHoursDisplay(kitchenSettings.customization_cutoff_hours)}</strong>
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* Stop Thaali Cutoff Section */}
                  <div className="card mb-4 border-0 shadow-sm">
                    <div
                      className="card-header"
                      style={{ background: '#198754', color: '#fff', borderRadius: '8px 8px 0 0 !important' }}
                    >
                      <h6 className="mb-0 fw-bold">
                    ...
                    <div className="card-body p-4">
                      ...
                      <div className="mb-3">
                        ...
                        <div className="d-flex flex-wrap gap-2">
                          {CUTOFF_PRESETS.map((h) => (
                            <button
                              key={h}
                              onClick={() => setKitchenSettings({ ...kitchenSettings, stop_thaali_cutoff_hours: h })}
                              className="btn btn-sm"
                              style={{
                                borderRadius: 20,
                                fontSize: 13,
                      ...
                      >
                        <div style={{ fontSize: 13, color: 'var(--bs-info-text-emphasis)' }}>
                          <i className="bi bi-info-circle me-2"></i>
                          <strong>Current: {kitchenSettings.stop_thaali_cutoff_hours} hours</strong> before delivery
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--bs-info-text-emphasis)', marginTop: 6, paddingLeft: 22 }}>
                          Example: for a <strong>Wednesday</strong> delivery, stop request must be submitted{' '}
                          <strong>{formatHoursDisplay(kitchenSettings.stop_thaali_cutoff_hours)}</strong>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="alert alert-warning mt-3 mb-0" style={{ fontSize: '12px' }}>
                    ...
                    <strong>Note:</strong> Legacy time-based settings (<code>customization_cutoff_hour/minute</code>) are deprecated.
                  </div>
                </div>
                    ...
                    <table className="table table-hover" style={{ fontSize: '13px' }}>
                      ...
                      <tbody>
                        {thaaliNumbers.length === 0 ? (
                          <tr>
                            <td colSpan={3} className="text-center py-4" style={{ color: 'var(--bs-secondary-color)' }}>
                              No thaali numbers added yet
                            </td>
                          </tr>
                        ) : (
                          thaaliNumbers.map((t) => (
                            <tr key={t.id}>
                              ...
                              <td>
                                {t.assigned ? (
                                  <span className="badge" style={{ background: '#36457420', color: '#364574', fontWeight: 600, fontSize: '11px' }}>
                                    Assigned
                                  </span>
                                ) : (
                                  <span className="badge" style={{ background: '#0ab39c20', color: '#0ab39c', fontWeight: 600, fontSize: '11px' }}>
                              ...
                              <td>
                                <button
                                  onClick={() => deleteThaali(t.id)}
                                  disabled={t.assigned}
                                  title={t.assigned ? 'Cannot delete — assigned to a mumin' : 'Delete'}
                                  className="btn btn-sm"
                                  style={{
                                    padding: '3px 8px',
                                    background: t.assigned ? 'transparent' : '#e6394615',
                                    color: t.assigned ? 'var(--bs-secondary-color)' : '#e63946',
                                    border: 'none',
                                    borderRadius: '6px',
                                  }}
                                >
                    ...
                    sectors.map((sec) => (
                      <div key={sec.id} className="mb-2" style={{ border: '1px solid var(--bs-border-color)', borderRadius: 10, overflow: 'hidden' }}>
                        <div className="d-flex align-items-center px-3 py-2" style={{ background: 'var(--bs-body-bg)' }}>
                          <span className="fw-semibold me-auto" style={{ color: 'var(--bs-body-color)' }}>
                            {sec.name}
                          </span>
                          <button
                  ...
                  <div className="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                    ...
                    <div className="d-flex gap-2 align-items-center">
                      ...
                      <button
                        ...
                        disabled={generatingPDF || !selectedStickers.length}
                        className="btn btn-sm"
                        style={{ background: '#364574', color: '#fff', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}
                      >
                        {generatingPDF ? (
                    ...
                    <table className="table table-hover" style={{ fontSize: '13px' }}>
                      ...
                      <tbody>
                        {filteredStickers.length === 0 ? (
                          <tr>
                            <td colSpan={4} className="text-center py-4" style={{ color: 'var(--bs-secondary-color)' }}>
                          ...
                          filteredStickers.map((r: any) => (
                            <tr key={r.id} onClick={() => toggleSticker(r.id)} style={{ cursor: 'pointer' }}>
                              <td>
                                <input
                                  type="checkbox"
                                  checked={selectedStickers.includes(r.id)}
                                  onChange={() => toggleSticker(r.id)}
                ...
                <div className="table-responsive">
                  <table className="table table-hover" style={{ fontSize: '13px' }}>
                    <thead style={{ background: 'var(--bs-tertiary-bg)', borderBottom: '2px solid var(--bs-border-color)' }}>
                      <tr>
                        ...
                        {activeTab === 'mumin_categories' && (
                          <th style={{ color: 'var(--bs-secondary-color)', fontWeight: 700 }}>Description</th>
                        )}
                    ...
                    <tbody>
                      {rows.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="text-center py-4" style={{ color: 'var(--bs-secondary-color)' }}>
                            Nothing added yet
                          </td>
                        </tr>
                      ) : (
                        rows.map((row, i) => (
                          <tr key={row.id}>
                            ...
                            {activeTab === 'mumin_categories' && (
                              <td style={{ fontSize: 12, color: 'var(--bs-secondary-color)' }}>{row.description || '—'}</td>
                            )}
                            <td>
                              <div className="d-flex gap-1">
                                <button
      ...
      {showModal && (
        ...
        >
          <div
            style={{ background: 'var(--bs-body-bg)', borderRadius: '12px', width: '100%', maxWidth: '440px', boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div
              style={{ padding: '16px 20px', borderBottom: '1px solid var(--bs-border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
            >
              <h6 className="mb-0 fw-bold" style={{ color: 'var(--bs-body-color)' }}>
              ...
              <button
                onClick={() => setShowModal(false)}
                style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: 'var(--bs-secondary-color)' }}
              >
                ×
              </button>
            </div>
            <div style={{ padding: '20px' }}>
              <div className="mb-3">
                <label className="form-label" style={{ fontSize: '13px', fontWeight: 600, color: 'var(--bs-body-color)' }}>
                  Name *
                </label>
                <input
                  type="text"
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--bs-border-color)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowModal(false)} className="btn btn-sm btn-outline-secondary" style={{ borderRadius: 8, fontSize: 13 }}>
                Cancel
              </button>
              <button
                onClick={saveRow}
      ...
      {showThaaliModal && (
        ...
        >
          <div
            style={{ background: 'var(--bs-body-bg)', borderRadius: 12, width: '100%', maxWidth: 440, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div
              style={{ padding: '16px 20px', borderBottom: '1px solid var(--bs-border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
            >
              ...
              <button
                onClick={() => setShowThaaliModal(false)}
                style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--bs-secondary-color)' }}
              >
                ×
              </button>
            </div>
            <div style={{ padding: 20 }}>
              <label className="form-label" style={{ fontSize: 13, fontWeight: 600, color: 'var(--bs-body-color)' }}>
                Thaali Numbers *
              </label>
              <input
                type="text"
                ...
                autoFocus
              />
              <p className="mt-2 mb-0" style={{ fontSize: 12, color: 'var(--bs-secondary-color)' }}>
                Separate multiple numbers with commas
              </p>
            </div>
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--bs-border-color)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowThaaliModal(false)} className="btn btn-sm btn-outline-secondary" style={{ borderRadius: 8, fontSize: 13 }}>
                Cancel
              </button>
              <button
                onClick={saveThaaliNumber}
                disabled={saving}
                className="btn btn-sm"
                style={{ background: '#364574', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600 }}
              >
