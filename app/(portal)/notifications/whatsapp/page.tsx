    ...
    <div style={{ padding: '24px 0', maxWidth: 640 }}>
      ...
      <div className="mb-4" style={{ borderBottom: '1px solid var(--bs-border-color)' }}>
        {([
          ...
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            color: tab === t.key ? theme.primary : 'var(--bs-secondary-color)',
            borderBottom: tab === t.key ? '2px solid var(--fmb-primary)' : '2px solid transparent',
            marginBottom: '-1px', transition: 'all 0.15s',
          }}>
            <i className={`bi ${t.icon} me-2`} style={{ color: tab === t.key ? theme.accent : 'inherit' }} />
            {t.label}
          </button>
        ))}
      </div>
            ...
            {status && (
              <div>
                <div style={{
                  ...
                  border: `1px solid ${status.is_connected ? theme.success : theme.danger}`,
                  ...
                  {status.is_connected && (
                    <span style={{ fontSize: 12, color: theme.success }}>— Connected</span>
                  )}
                </div>
          ...
          <div style={{
            ...
            {qrLoading && !qrUrl && (
              <div className="text-center">
                <div className="spinner-border" style={{ color: theme.primary, width: '2rem', height: '2rem' }} />
                <div style={{ fontSize: 13, color: '#888', marginTop: 12 }}>Loading QR...</div>
              </div>
            )}
            {qrError && (
              <div className="text-center">
                <i className="bi bi-exclamation-triangle-fill" style={{ fontSize: 32, color: theme.warning }} />
                <div style={{ fontSize: 13, color: theme.danger, marginTop: 8 }}>{qrError}</div>
      ...
      {tab === 'templates' && (
        <div>
          ...
          <div style={{ background: 'var(--bs-body-bg)', border: '1px solid var(--bs-border-color)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
            <h6 className="fw-bold mb-3" style={{ color: 'var(--bs-body-color)', fontSize: 14 }}>
              <i className="bi bi-plus-circle-fill me-2" style={{ color: theme.primary }} />
              New Template
            </h6>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {VARIABLES.map(v => (
                  <button key={v.key} onClick={() => v.available && insertVariable(v.key)}
                    ...
                    style={{
                      border: '1px solid',
                      borderColor: v.available ? theme.primary : 'var(--bs-border-color)',
                      background: v.available ? 'rgba(54,69,116,0.08)' : 'var(--bs-tertiary-bg)',
                      color: v.available ? theme.primary : 'var(--bs-secondary-color)',
                ...
                <div key={t.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1, marginRight: 12 }}>
                      ...
                      {t.variables?.length > 0 && (
                        <div style={{ marginTop: 6, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          {t.variables.map(v => (
                            <span key={v} style={{ fontSize: 11, background: 'rgba(54,69,116,0.1)', color: theme.primary, borderRadius: 10, padding: '2px 8px' }}>
                              {`{{${v}}}`}
                            </span>
                          ))}
                        </div>
                    ...
                    <button onClick={() => deleteTemplate(t.id)} disabled={deletingId === t.id}
                      style={{ background: 'none', border: 'none', color: theme.danger, cursor: 'pointer', fontSize: 16, padding: 4 }}>
                      {deletingId === t.id
      ...
      {tab === 'campaigns' && (
        <div>
          <div style={{ background: 'var(--bs-body-bg)', border: '1px solid var(--bs-border-color)', borderRadius: 12, padding: 20, marginBottom: 16 }}>
            <h6 className="fw-bold mb-3" style={{ color: 'var(--bs-body-color)', fontSize: 14 }}>
              <i className="bi bi-megaphone-fill me-2" style={{ color: theme.primary }} />
              Queue Reminders
            </h6>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ...
                  <label key={s.key} style={{
                    ...
                    borderColor: campSegment === s.key ? theme.primary : 'var(--bs-border-color)',
          ...
          {queueResult && (
            <div style={{
              ...
              <div style={{ fontWeight: 700, fontSize: 14, color: theme.success, marginBottom: 10 }}>
        ...
        <div>
          ...
          {logsLoading && (
            <div className="text-center py-5">
              <div className="spinner-border" style={{ color: theme.primary, width: '1.5rem', height: '1.5rem' }} />
            </div>
          )}
              ...
              {logs.map((log, i) => (
                <div key={log.campaign_name} style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                    {[
                      { label: 'Total',   value: log.total,   color: 'var(--fmb-primary)' },
                  ...
                  {log.total > 0 && (
                    <div style={{ marginTop: 10, height: 4, background: 'var(--bs-tertiary-bg)', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${(log.sent / log.total) * 100}%`, background: theme.success, borderRadius: 4, transition: 'width 0.3s' }} />
                    </div>
                  )}
                </div>
              ))}
