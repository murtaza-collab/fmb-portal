...
export default function MumineenPage() {
  ...
  const handleSave = async () => {
    ...
    const payload: any = {
      ...
      mumin_category_id: form.mumin_category_id || null,
  ...
  const handleExport = () => {
    const headers = ['SF#', 'ITS#', 'Full Name', 'Phone', 'WhatsApp', 'Address', 'Sector', 'Category', 'Niyyat Status']
    const rows = filtered.map(m => [m.sf_no, m.its_no || '', m.full_name, m.phone_no || '', m.whatsapp_no || '', m.full_address || '', getSectorName(m.address_sector_id), getCategoryById(m.mumin_category_id)?.name || '', getNiyyatName(m.niyyat_status_id)])
    const csv = [headers, ...rows].map(r => r.map(v => `"${v}"`).join(',')).join('\n')
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'mumineen.csv'; a.click()
  }
            ...
            <>
              <table className="table table-hover mb-0" style={{ fontSize: '13px' }}>
                <thead style={{ background: '#f8f9fa' }}>
                  <tr>
                    {['SF#', 'ITS#', 'Full Name', 'Phone', 'WhatsApp', 'Address', 'Sector', 'Niyyat Status', 'Category', ''].map(h => (
          ...
          <div className="modal-body" style={{ overflowY: 'auto', maxHeight: '65vh' }}>
                ...
                <div className="row g-3">
                  ...
                  <div className="col-12">
                    ...
                    <div style={{ background: '#f8f9fa', borderRadius: '6px', padding: '8px 12px', fontSize: '13px', color: addressPreview ? '#364574' : '#adb5bd', borderLeft: `3px solid ${addressPreview ? '#ffbf69' : '#dee2e6'}`, minHeight: '36px', lineHeight: '20px' }}>
                ...
                {!editing && (
                  <div className="mt-3">
                    <small className="text-muted"><i className="bi bi-info-circle me-1" />Niyyat status will be set to <strong>No-Show</strong> by default.</small>
                  </div>
                )}
          </div>
