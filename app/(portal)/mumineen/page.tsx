      ...
      <div className="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
        <div>
          ...
          <p className="mb-0" style={{ fontSize: 13, color: 'var(--bs-secondary-color)' }}>
            ...
            <span style={{ color: '#d4a032', fontWeight: 600 }}>{hofs.filter(h => h.status === 'active').length}</span> Active HOFs ·{' '}
        ...
        <div className="d-flex gap-2 flex-wrap">
          ...
          {!(tab === 'hofs' && hofSubTab === 'transferred') && tab !== 'all' && (
            <button className="btn btn-sm" style={{ background: '#d4a032', color: '#fff' }} onClick={openAdd}><i className="bi bi-plus me-1" />Add HOF</button>
          )}
        </div>
      </div>
          ...
          {([
            ...
            <button key={key}
              ...
              style={{
                ...
                color: tab === key ? '#d4a032' : 'var(--bs-secondary-color)',
            ...
            {subItems.map(([key, label]) => (
              <button key={key} onClick={() => setSubTab(key)}
                style={{
                  ...
                  color: curSubTab === key ? '#d4a032' : 'var(--bs-secondary-color)',
                      ...
                      (paginated as Mumin[]).map(m => {
                        ...
                        return (
                          <tr key={m.id} style={{ opacity: m.status === 'transferred' ? 0.6 : 1 }}>
                            <td>
                              {m.is_hof
                                ? <span className="badge" style={{ background: '#d4a032', color: '#fff', fontSize: 11 }}>HOF</span>
                          ...
                          <tr key={m.id} style={{ opacity: m.status === 'transferred' ? 0.65 : 1 }}>
                            ...
                            <td>
                              <div className="d-flex gap-1 justify-content-end">
                                ...
                                <button className="btn btn-sm" title="Edit" style={{ padding: '2px 7px', color: '#d4a032' }} onClick={() => openEdit(m)}><i className="bi bi-pencil" /></button>
                      ...
                      (paginated as Mumin[]).map(m => {
                        ...
                        return (
                          <tr key={m.id}>
                            <td style={{ color: 'var(--bs-secondary-color)' }}>{m.its_no || '—'}</td>
                            <td style={{ fontWeight: 500, color: 'var(--bs-body-color)' }}>{m.full_name}</td>
                            <td style={{ color: '#d4a032', fontWeight: 600 }}>{m.sf_no || hof?.sf_no || '—'}</td>
                            <td style={{ color: 'var(--bs-secondary-color)', fontSize: 12 }}>{hof ? hof.full_name : '—'}</td>
                            <td style={{ color: 'var(--bs-secondary-color)', fontSize: 12 }}>{calcAge(m.dob)}</td>
                            <td style={{ color: 'var(--bs-secondary-color)' }}>{m.phone_no || '—'}</td>
                            <td>
                              <div className="d-flex gap-1 justify-content-end">
                                {hof && memberSubTab === 'active' && <button className="btn btn-sm" title="Edit" style={{ padding: '2px 7px', color: '#d4a032' }} onClick={() => openEditMember(m as any, hof)}><i className="bi bi-pencil" /></button>}
      ...
      {showModal && (
        <Modal onClose={() => setShowModal(false)} size="modal-lg">
          ...
          <div className="modal-body" style={{ overflowY: 'auto', maxHeight: '65vh' }}>
            ...
            <div className="row g-3">
              ...
              <div className="col-12">
                ...
                <div style={{ background: 'var(--bs-secondary-bg)', borderRadius: 6, padding: '8px 12px', fontSize: 13, color: addressPreview ? '#d4a032' : 'var(--bs-secondary-color)', borderLeft: `3px solid ${addressPreview ? '#ffd97d' : 'var(--bs-border-color)'}`, minHeight: 36 }}>
          ...
          <div className="modal-footer" style={{ borderTop: '1px solid var(--bs-border-color)' }}>
            ...
            <button className="btn btn-sm" style={{ background: '#d4a032', color: '#fff' }} onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : editing ? 'Save Changes' : 'Save'}</button>
          </div>
        </Modal>
      )}
      ...
      {showFamilyModal && familyModalHof && (
        <Modal onClose={() => setShowFamilyModal(false)}>
          ...
          <div className="modal-footer" style={{ borderTop: '1px solid var(--bs-border-color)' }}>
            ...
            <button className="btn btn-sm" style={{ background: '#d4a032', color: '#fff' }} onClick={handleSaveMember} disabled={savingMember}>{savingMember ? 'Saving...' : 'Save'}</button>
          </div>
        </Modal>
      )}
    </>
