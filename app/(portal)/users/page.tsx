...
const PERM_LABELS: Record<string, string> = {
  can_view: 'View', can_add: 'Add', can_edit: 'Edit', can_deactivate: 'Deactivate'
}
...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    const { data } = await supabase.from('admin_users')
      .select('*, user_groups(name)').order('full_name')
    setUsers(data || [])
    setLoading(false)
  }
  ...
  const openEditUser = (u: AdminUser) => {
    ...
    setUserForm({
      ...
      username: u.username || '',
      password: '',
      user_group_id: u.user_group_id?.toString() || ''
    })
    setFormError('')
    setShowUserModal(true)
  }
  ...
  const deleteUser = async (u: AdminUser) => {
    if (!confirm(`Are you sure you want to permanently delete "${u.full_name}"? This cannot be undone.`)) return
  ...
  const initPerms = (): Record<string, Record<string, boolean>> => {
    ...
    MODULES.forEach(m => {
      p[m.key] = { can_view: false, can_add: false, can_edit: false, can_deactivate: false }
    })
    return p
  }
    ...
    ;(g.permissions || []).forEach(perm => {
      if (p[perm.module]) {
        p[perm.module] = {
          ...
          can_deactivate: perm.can_deactivate,
        }
      }
    })
    setGroupPerms(p)
  ...
  const toggleAllModule = (module: string) => {
    ...
    setGroupPerms(prev => ({
      ...prev,
      [module]: { can_view: !allOn, can_add: !allOn, can_edit: !allOn, can_deactivate: !allOn }
    }))
  }
  ...
  const handleSaveGroup = async () => {
    ...
    if (editingGroup) {
      ...
      const { data } = await supabase.from('user_groups')
        .insert({ name: groupName, status: 'active' })
        .select('id').single()
      groupId = data!.id
    }
    ...
    const rows = MODULES.map(m => ({
      ...
      module: m.key,
      can_view: groupPerms[m.key].can_view,
      can_add: groupPerms[m.key].can_add,
      can_edit: groupPerms[m.key].can_edit,
      can_deactivate: groupPerms[m.key].can_deactivate,
    }))
  ...
  const toggleGroupStatus = async (g: UserGroup) => {
    await supabase.from('user_groups')
      .update({ status: g.status === 'active' ? 'inactive' : 'active' })
      .eq('id', g.id)
    await fetchGroups()
  }

  const deleteGroup = async (g: UserGroup) => {
    if (!confirm(`Are you sure you want to permanently delete group "${g.name}"? This cannot be undone.`)) return
      ...
      <ul className="nav nav-tabs mb-3">
        <li className="nav-item">
          <button className={`nav-link ${activeTab === 'users' ? 'active' : ''}`}
            onClick={() => setActiveTab('users')}>Admin Users</button>
        </li>
        <li className="nav-item">
          <button className={`nav-link ${activeTab === 'groups' ? 'active' : ''}`}
            onClick={() => setActiveTab('groups')}>User Groups & Permissions</button>
        </li>
      </ul>
                ...
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <div>
                    ...
                    <small className="text-muted">
                      {(g.permissions || []).filter(p => p.can_view).length} modules accessible
                    </small>
                  </div>
                  <div className="d-flex gap-2 align-items-center">
                    ...
                    <button
                      className={`btn btn-sm ${g.status === 'active' ? 'btn-outline-warning' : 'btn-outline-success'}`}
                      style={{ fontSize: '12px' }} onClick={() => toggleGroupStatus(g)}>
                      {g.status === 'active' ? 'Deactivate' : 'Activate'}
                    </button>
                    <button className="btn btn-sm btn-outline-danger"
                      style={{ fontSize: '12px' }} onClick={() => deleteGroup(g)}>Delete</button>
                  </div>
                </div>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-body">
                {formError && (
                  <div className="alert alert-danger py-2" style={{ fontSize: '13px' }}>{formError}</div>
                )}
                    ...
                    <tr>
                      ...
                      {PERM_KEYS.map(k => (
                        <th key={k} style={{ textAlign: 'center', cursor: 'pointer' }}
                          ...
                          {PERM_LABELS[k]}
                          <div style={{ fontSize: '10px', color: '#0d6efd' }}>toggle all</div>
                        </th>
                      ))}
